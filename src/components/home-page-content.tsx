...
import React, { useEffect, useState, useRef, ComponentType, lazy, Suspense } from 'react';
...
const DynamicLucideIcon = ({ name }: { name: string }) => {
  ...
  return <Icon className="h-5 w-5 text-muted-foreground hover:text-foreground" />;
};
      ...
      <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="container mx-auto flex h-14 items-center justify-between px-4 md:px-6">
          <div className="flex items-center gap-2">
            <Image src={settings.branding.logoUrl} alt={`${settings.branding.siteName} logo`} width={32} height={32} />
            ...
            <div className="grid gap-8 md:grid-cols-3">
              ...
              <ScrollAnimation delay={0.2}>
                <div className="flex flex-col items-center">
                    ...
                    <p className="text-muted-foreground">Share your unique BookWise link on your website, social media, or directly with clients.</p>
                </div>
              </ScrollAnimation>
              <ScrollAnimation delay={0.3}>
                <div className="flex flex-col items-center">
          ...
          <div className="container mx-auto px-4 text-center md:px-6">
            ...
            <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
              {[
                ...
                { icon: CalendarCheck, title: 'Google Calendar Sync', desc: 'Sync your BookWise schedule with your Google Calendar to avoid any booking conflicts.' },
                  ...
                  <div className="grid gap-8 md:grid-cols-2">
                    <ScrollAnimation delay={0.1}>
                      <div className="overflow-hidden rounded-lg border shadow-lg">
                          <Image
                          src={settings.screenshots.image1Url}
                          alt="Provider Dashboard Screenshot"
                          width={1200}
                          height={800}
                          className="w-full"
                          data-ai-hint="dashboard bookings"
                          />
                      </div>
                    </ScrollAnimation>
                    <ScrollAnimation delay={0.2}>
                      <div className="overflow-hidden rounded-lg border shadow-lg">
                          <Image
                          src={settings.screenshots.image2Url}
                          alt="Public Booking Page Screenshot"
                          width={1200}
                          height={800}
                          className="w-full"
                    ...
                    {settings.faq && settings.faq.length > 0 ? (
                        ...
                        <>
                             <AccordionItem value="item-1">
                                ...
                                <AccordionContent>
                                Yes, absolutely! We offer a 3-day free trial with full access to all features. No credit card is required to get started, so you can explore everything BookWise has to offer risk-free.
                                </AccordionContent>
                            </AccordionItem>
                            <AccordionItem value="item-2">
                                <AccordionTrigger>Can customers cancel or reschedule through BookWise?</AccordionTrigger>
                                <AccordionContent>
        ...
        <section className="bg-primary text-primary-foreground py-20">
          <div className="container mx-auto px-4 text-center md:px-6">
            <ScrollAnimation>
                ...
                Join hundreds of professionals who are saving time, reducing no-shows, and growing their business with BookWise.
                </p>
            </ScrollAnimation>
            <ScrollAnimation delay={0.1}>
                <div className="mt-8">
        ...
        <div className="container mx-auto px-4 py-12 md:px-6">
            <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-4">
                <div className="lg:col-span-2">
                    ...
                    {settings.footer?.description && <p className="mt-4 text-muted-foreground">{settings.footer.description}</p>}
                </div>
                <div>
                    {settings.footer?.siteLinks && settings.footer.siteLinks.length > 0 && (
                        <>
                         <h4 className="font-semibold">Links</h4>
                         <ul className="mt-4 space-y-2">
                            {settings.footer.siteLinks.map(link => (
                                <li key={link.id}><Link href={link.url} onClick={e => scrollLink(e, link.url)} className="text-sm text-muted-foreground hover:text-foreground">{link.name}</Link></li>
                            ))}
                        </ul>
                        </>
                    )}
                 ...
                 <div>
                    {settings.footer?.contact && (
                        ...
                        <ul className="mt-4 space-y-2 text-sm text-muted-foreground">
                            {settings.footer.contact.email && <li><a href={`mailto:${settings.footer.contact.email}`} className="hover:text-foreground">{settings.footer.contact.email}</a></li>}
             ...
             <div className="flex flex-col items-center justify-between gap-4 md:flex-row">
                <p className="text-sm text-muted-foreground">{settings.footer?.copyright || `© ${new Date().getFullYear()} ${settings.branding.siteName}. All rights reserved.`}</p>
                 {settings.footer?.socialLinks && settings.footer.socialLinks.length > 0 && (
                     <div className="flex items-center gap-4">
                        {settings.footer.socialLinks.map(link => (
                            <Link key={link.id} href={link.url} aria-label={link.name}>
