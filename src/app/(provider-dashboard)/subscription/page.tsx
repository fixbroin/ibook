...
import { updateProviderSubscription, createRazorpayOrder, verifyRazorpayPayment } from '@/lib/actions';
...
function SubscriptionComponent() {
  ...
  useEffect(() => {
    ...
    if (razorpay_payment_id && razorpay_order_id && razorpay_signature && planId && user?.email) {
      const username = user.email.split('@')[0];
      
      const verifyPaymentAndSubscribe = async () => {
        setIsVerifying(true);
        try {
            const { isAuthentic } = await verifyRazorpayPayment({ razorpay_payment_id, razorpay_order_id, razorpay_signature });

            if (isAuthentic) {
                const result = await updateProviderSubscription(username, planId, { razorpay_payment_id, razorpay_order_id, razorpay_signature });
                if (result.success && result.provider) {
                    toast({ title: 'Payment Successful', description: 'Your subscription has been activated!'});
                    setProvider(result.provider); // Directly update state with the new provider data
                } else {
  ...
  const handleChoosePlan = async (plan: Plan) => {
    ...
    startPaymentTransition(async () => {
      ...
      const order = await createRazorpayOrder(plan.price, 'INR', plan.id);
      if (!order) {
      ...
      const options = {
        ...
        name: "BookWise Subscription",
        description: `Payment for ${plan.name}`,
        image: "/icon.png", 
        order_id: order.id,
        callback_url: `${window.location.origin}/subscription?planId=${plan.id}`,
        handler: async function (response: any){
          setIsVerifying(true);
          const { isAuthentic } = await verifyRazorpayPayment(response);
          
          if (isAuthentic) {
            const result = await updateProviderSubscription(username, plan.id, response);
            if (result.success && result.provider) {
              toast({ title: 'Payment Successful', description: 'Your subscription has been activated!'});
              setProvider(result.provider); // Re-fetch data to update UI
            } else {
              toast({ title: 'Update Failed', description: result.error, variant: 'destructive'});
            }
          } else {
