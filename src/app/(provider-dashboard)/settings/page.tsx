...
export default function SettingsPage() {
  ...
  const handleServiceTypeChange = (type: string, checked: boolean) => {
    if (!provider) return;
    const currentTypes = provider.settings.serviceTypes;
    const newTypes = checked ? [...currentTypes, type] : currentTypes.filter(t => t !== type);
    handleSettingsChange('serviceTypes', newTypes);
  }
    ...
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Define the types of services you offer and where they take place.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
            <div className="space-y-2">
                {['Online', 'Shop Visit', 'Doorstep'].map(type => (
                    <div key={type} className="flex items-center space-x-2">
                        <Checkbox id={`service-${type}`} checked={settings.serviceTypes.includes(type as any)} onCheckedChange={(checked) => handleServiceTypeChange(type, !!checked)} />
            ...
            {orderedDays.map((day) => {
              ...
              return (
                 <div key={day} className="grid grid-cols-3 items-center gap-4">
                    <div className="flex items-center space-x-2">
                        <Checkbox id={day} checked={isEnabled} onCheckedChange={(checked) => handleWorkingHoursChange(day, 'enabled', !!checked)} />
