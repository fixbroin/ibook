...
import { getBookingsByProvider, getProviderByUsername, updateBookingStatus, deleteBooking, getBookingsForDay } from "@/lib/data";
...
import { Eye, Check, X, Trash2, Loader2, Calendar, Clock, User, Mail, Phone, MapPin, Globe, CalendarPlus } from "lucide-react";
...
export default function BookingsPage() {
  ...
  const fetchData = async (username: string) => {
      ...
      try {
        ...
        const enrichedBookings = bookingsData.map(b => {
            ...
            // If the booking is in the past, and still marked 'Upcoming', it should be 'Not Completed'.
            if (b.dateTime < new Date() && (status === 'Upcoming')) {
  ...
  const openRescheduleDialog = (booking: EnrichedBooking) => {
    setRescheduleDialogState({ open: true, booking });
  };
                    ...
                    <h3 className="font-semibold text-lg pt-2">Appointment Information</h3>
                     <div className="space-y-3 rounded-lg border bg-muted/50 p-4">
                        <div className="flex items-center gap-3">
                         ...
                         {booking.serviceType === 'Online' && !booking.address && (
                             <div className="flex items-start gap-3">
                                <MapPin className="h-5 w-5 text-muted-foreground mt-1" />
                                <div>
                                    <p className="font-medium">{booking.serviceType}</p>
  ...
  const RescheduleBookingDialog = () => {
    ...
    const timeSlots = useMemo(() => {
        ...
        return potentialSlots.map(time => {
            ...
            // We exclude the current booking being rescheduled from the count
