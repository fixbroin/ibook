...
export default function AdminTestimonialsPage() {
  ...
  const handleSave = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      let testimonialData = { ...currentTestimonial };
      ...
      const idInput = formElements.namedItem('id') as HTMLInputElement;
      const nameInput = formElements.namedItem('name') as HTMLInputElement;
      const ratingInput = formElements.namedItem('rating') as HTMLSelectElement;
      const descriptionInput = formElements.namedItem('description') as HTMLTextAreaElement;
      const imageUrlInput = formElements.namedItem('imageUrlInput') as HTMLInputElement;

      formData.append('id', idInput?.value || '');
      formData.append('name', nameInput?.value || '');
      formData.append('rating', ratingInput?.value || '5');
      formData.append('description', descriptionInput?.value || '');
      formData.append('imageUrlInput', imageUrlInput?.value || '');
  ...
  const handleDelete = () => {
    ...
    startTransition(async () => {
      const result = await deleteTestimonial(currentTestimonial.id!);
      if(result.success) {
          ...
          <form onSubmit={handleSave} ref={formRef}>
             <ScrollArea className="max-h-[70vh] p-1">
              <div className="space-y-4 p-4">
                  <input type="hidden" name="id" defaultValue={currentTestimonial?.id || ''} />
