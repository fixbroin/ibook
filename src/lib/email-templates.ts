...
export async function sendSubscriptionEmail(to: string, name: string, planName: string, expiryDate: Date, isRenewal: boolean) {
    if (isRenewal) {
         await sendEmail({
            to,
            subject: 'Your BookWise Subscription has been Renewed',
        ...
        await sendEmail({
            to,
            subject: 'Your BookWise Subscription is Active!',
...
export async function sendAccountStatusEmail(to: string, name: string, isSuspended: boolean) {
    ...
    const status_message = isSuspended 
        ? "Your BookWise account has been suspended. You will not be able to log in or receive new bookings. Please contact support for more information."
        : "Your BookWise account has been reinstated. You can now log in and continue using our services. Welcome back!";
...
export async function sendExpiryReminderEmail(to: string, name: string, planName: string, daysRemaining: number) {
     await sendEmail({
        to,
        subject: 'Your BookWise Subscription is Expiring Soon',
...
export async function sendPlanExpiredEmail(to: string, name: string, planName: string) {
      await sendEmail({
        to,
        subject: 'Your BookWise Subscription Has Expired',
