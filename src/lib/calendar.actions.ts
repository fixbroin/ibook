...
export async function createGoogleCalendarEvent(provider: Provider, booking: Booking): Promise<string | null> {
    ...
    try {
        ...
        const event = {
            ...
            attendees: [
                ...
                { email: provider.contact.email, displayName: provider.name }
            ],
            reminders: {
                useDefault: false,
                overrides: [
